...
function Lieu(options) {
    ...
    this.position = options.position || positionDefaut;
    ...
    this.serveurs = (options.serveurs || []).map(function(serveur) {
        return new Serveur(serveur);
    });
}
...
Lieu.prototype.element = function() {
    ...
    var position = document.createElement("button");
    position.className = "changePosition";
    position.onclick = afficheCarte.bind(null, this);
...
/* calcule la distance entre une position et la sienne (Pythagore) */
Lieu.prototype.calculeDistance = function(position) {
    var dlat = this.position[0] - position[0];
    var dlng = this.position[1] - position[1];
