...
/* n'indique plus la fin de partie */
Gomoku.prototype.annuleFin = function() {
    this.coupAuthorise = true;
};

/* ré-initialise la partie  */
Gomoku.prototype.recommence = function() {
    this.annuleFin();
    this.plateau = [];
...
Gomoku.prototype.joueCoup = function(x, y) {
    if (this.coupAuthorise && this.couleurCase(x, y) === 0) {
        ...
        this.enregistreCoup(x, y);
        this.tourDeJeu = -this.tourDeJeu;
...
/* permet de mettre le coup dans l'historique */
Gomoku.prototype.enregistreCoup = function(x, y) {
    //suprime tous les coups enregistrés après celui-ci
    this.coups = this.coups.slice(0, this.numeroCoup);
    this.coups[this.numeroCoup] = [x, y, this.tourDeJeu];
    this.numeroCoup++;
};

/* revient un coup en arrière */
Gomoku.prototype.reculeCoup = function() {
    var x, y;
    if (this.numeroCoup > 0) {
        this.numeroCoup--;
        x = this.coups[this.numeroCoup][0];
        y = this.coups[this.numeroCoup][1];
        this.changePierre(x, y, 0);
        this.plateau[x][y] = 0;
        this.tourDeJeu = this.coups[this.numeroCoup][2];
        this.annuleFin();
    }
};

/* avance d'un coup dans l'historique */
Gomoku.prototype.avanceCoup = function() {
    var x, y, couleur;
    if (this.numeroCoup < this.coups.length) {
        x = this.coups[this.numeroCoup][0];
        y = this.coups[this.numeroCoup][1];
        couleur = this.coups[this.numeroCoup][2];
        this.changePierre(x, y, couleur);
        this.plateau[x][y] = couleur;
        this.tourDeJeu = -couleur;
        this.numeroCoup++;
        this.verifieFin(x, y);
    }
};
